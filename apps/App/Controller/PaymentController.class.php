...
class PaymentController extends BaseController {
    ...
    private $pay_state = array('wx'=>'微信','alipay'=>'支付宝','newalipay'=>'新支付宝');
    ...
    public function routepay(){
        ...
        /* 如果传了 return_se_url 或者 return_er_url 则使用这两个地址做为回跳页面 */
        if(!empty($_GET['return_se_url'])){
            $return_url['finishUrl'] = urldecode(base64_decode($_GET['return_se_url']));
        }
        if(!empty($_GET['return_er_url'])){
            $return_url['undoneUrl'] = urldecode(base64_decode($_GET['return_er_url']));
        }
    ...
    public function pay(){
		...
		switch($this->payment_code) {
            ...
            case 'alipay':
                ...
                if(!empty($return_url['finishUrl'])){
                    $param['return_url'] = SITE_URL ."api.php/Payment/alipayReturnOther";
                }
                /*实例化支付宝类*/
                $alipay = new \Common\PayMent\AliPay\Alipays($param);
                $this->writeLog(var_export($alipay->alipay_config,true));
                /*渲染建立请求的页面*/
    ...
    /* 支付宝支付成功之后的跳转 其它来源的订单，需要跳转到订单里设置的回跳地址 */
    public function alipayReturnOther(){
        $order_sn = $_GET['out_trade_no'];
        if(empty($this->pay_class_method)){
            $this->_get_order_class($order_sn);
        }
        ...
        $alipay = new \Common\PayMent\AliPay\Alipays($this->payment_config);
        /* 传成功的url地址，失败的url地址 */
        //查询订单是否有回跳地址，有的话则使用该地址跳转
        $order_info = M('orders')->where(array('order_sn'=>$order_sn))->find();
        if(!empty($order_info['return_se_url'])){
            $succeeurl = urldecode(base64_decode($order_info['return_se_url']));
        }else{
            $succeeurl = trim(SITE_URL,'/') .$this->pay_class_method['succeeurl'];
        }
        if(!empty($order_info['return_er_url'])){
            $failurl = urldecode(base64_decode($order_info['return_er_url']));
        }else{
            $failurl = trim(SITE_URL,'/') .$this->pay_class_method['failurl'];
        }
        $result = $alipay->alipayReturn($succeeurl, $failurl);
