...
class GoodsModel extends Model {
	...
	public function  getSpuById($goods_common_id){
	   ...
	   if(!empty($cache)){
    	   ...
    	   $goods_list =   M('goods')->field('goods_id,goods_name,goods_state,goods_price,goods_storage,goods_image,spec_value_id_key,goods_spec,freight')->where(array('goods_common_id'=>$goods_common['goods_common_id']))->select();
    	   $deafult_img = M('setting')->where(array('name' => 'shop_goods'))->find();
    	   $goods_storage =  0;
           $goods_ids = array();
           ...
           // 获取商品销量
           $goods_sales = $this->getGoodsSales($goods_ids);
    	   ...
    	   foreach($goods_list as $k=>$v){
               ...
               if(!empty($goods_sales[$v['goods_id']])){
                    $goods_common['goods_list'][$k]['goods_sales'] = $goods_sales[$v['goods_id']];
                    $goods_common['goods_sales'] += $goods_sales[$v['goods_id']];
               }else{
                    $goods_common['goods_list'][$k]['goods_sales'] = 0;
               }
    	   }
           ...
           $goods_common['goods_comment_count'] = M('evaluateGoods')->where($where)->count();
    	   return $goods_common;
	   }
	}
     ...
     * 取商品销量
     */
    public function getGoodsSales($goods_ids){
        $goods_sales = M('order_goods')->field('goods_id,sum(goods_num) as goods_num')->where(array('goods_id'=>array('in',$goods_ids)))->order('goods_id desc')->group('goods_id')->select();
        $goods_sales_list = array();
        foreach ($goods_sales as $key => $value) {
            $goods_sales_list[$value['goods_id']] = $value['goods_num'];
        }
