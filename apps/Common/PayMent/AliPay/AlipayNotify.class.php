...
class AlipayNotify extends AliFunction {
	...
	public function verifyNotify($post){
		...
		file_put_contents("/data/www/web/www.tqshq.com/data/Logs/alipay_notify.log",'isSign======='.$isSign.PHP_EOL,FILE_APPEND);	
			//获取支付宝远程服务器ATN结果（验证是否是支付宝发来的消息）
			$responseTxt = 'false';
			if (! empty($post["notify_id"])) {$responseTxt = $this->getResponse($post["notify_id"]);}
			//写日志记录
			
		file_put_contents("/data/www/web/www.tqshq.com/data/Logs/alipay_notify.log",'responseTxt======='.$isSign.PHP_EOL,FILE_APPEND);	
