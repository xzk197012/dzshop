	    ...
	    <div class="jurisdiction boxsizing release-juris">
	        <dl class="juris-dl boxsizing release-dl">
				...
				<dd class="left text-l">
					...
					<?php if(!empty($spec_list)){ foreach($spec_list as $v){ ?> <p class="radiobox release-check"><input   <?php if(array_key_exists($v['spec_id'], $goods_common['spec'])){ echo 'checked="checked"';} ?>   class="regular-radio spec_lists"  type="checkbox" value="<?php echo $v['spec_id']?>"  id="spec_lists_<?php echo $v['spec_id']?>"   name="spec_ids[]"/><label for="spec_lists_<?php echo $v['spec_id']?>"></label><span  title="<?php echo $v['spec_name'];?>" class="radio-word"><?php echo $v['spec_name'];?></span></p><?php } } ?>
					</div>
