...
<div class="iframeMain">
    <ul class="transverse-nav">
        <li class="<?php echo ($status==''?'activeFour':''); ?>"><a href="<?php echo U('Presales/returns/status/');?>">全部</a></li>
        <li class="<?php echo ($status=='1'?'activeFour':''); ?>"><a href="<?php echo U('Presales/returns/status/1');?>">待处理</a></li>
        <li class="<?php echo ($status=='2'?'activeFour':''); ?>"><a href="<?php echo U('Presales/returns/status/2');?>">待退款</a></li>
        <li class="<?php echo ($status=='3'?'activeFour':''); ?>"><a href="<?php echo U('Presales/returns/status/3');?>">已退货</a></li>
        <li class="<?php echo ($status=='4'?'activeFour':''); ?>"><a href="<?php echo U('Presales/returns/status/4');?>">已拒绝</a></li>
    </ul>
    <div class="white-bg">
        <div class="table-titbox">
            ...
            <form method="get" id='formid' class="form-horizontal" name="Search_form"  action="<?php echo U('Presales/returns');?>">
            ...
            <table class="com-table">
                ...
                <tbody>
                    ...
                    <?php if(is_array($list)): foreach($list as $key=>$vo): ?><tr>
                        <td>
                            <div class="table-iconbox">
                                <?php if($vo["status_code"] == '1' ): ?><div class="edit-iconbox left edit-sele marginR10">
                                        <a class="edit-word table-icon-a" href='<?php echo U("Presales/editreturn/id/$vo[rg_id]");?>'><i class="table-com-icon table-handle-icon"></i><span class="gap">审核</span></a>
                                    </div>
                                <?php elseif($vo["status_code"] == '2' ): ?>
                                    <div class="edit-iconbox left edit-sele marginR10">
                                        <a class="edit-word table-icon-a" href='<?php echo U("Presales/returndetail/id/$vo[rg_id]/status/return");?>' data-return_id=<?php echo ($vo[rg_id]); ?>><i class="table-com-icon table-refund-icon"></i><span class="gap">退款</span></a>
                                    </div>
                                <?php else: ?>
                                    <div class="edit-iconbox left edit-sele marginR10">
                                        <a class="edit-word table-icon-a" href='<?php echo U("Presales/returndetail/id/$vo[rg_id]/status/details");?>'><i class="table-com-icon table-look-icon"></i><span class="gap">详情</span></a>
                                    </div><?php endif; ?>
                            </div>
                        </td>
                        ...
                        <td class="text-l"><div class="word-overflow" title="<?php echo ($vo['goods_name']); ?>"><?php echo ($vo['goods_name']); ?></div></td>
                        <td class="text-l"><?php echo ($vo["return_amount"]); ?></td>
                        <td class="text-l"><?php echo ($vo['causes_name']); ?></td>
                        <td class="text-l"><?php echo ($vo['status']); ?></td>
                        <td class="text-l"><div class="word-overflow" title="<?php echo ($vo['remark']); ?>"><?php echo ($vo['remark']); ?></div></td>
...
<script type="text/javascript">
    ...
    function area_del(id) {
        showSuccess('您确定要打款给买家吗',function () {
            window.location.href="<?php echo U('Presales/return_pay');?>"+"?id="+return_id;
        });
    }
</script>
    </div>
