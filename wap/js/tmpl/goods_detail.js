...
//　处理商品运费、库存、销量
function setGoodsOther(){
	var goods_other = {};
	if(typeof now_goods.goods_storage == 'undefined'){
		// 当前没有选中任何商品规格
		goods_other.freight = goods_common.freight;
		goods_other.goods_storage = goods_common.goods_storage;
		goods_other.goods_sales = goods_common.goods_sales;
	}else{
		goods_other.freight = now_goods.freight;
		goods_other.goods_storage = now_goods.goods_storage;
		goods_other.goods_sales = now_goods.goods_sales;
	}
	$('#goods_freight').text(returnFloat(goods_other.freight));
	$('#goods_storage').text(goods_other.goods_storage);
	$('#goods_sales').text(goods_other.goods_sales);
}
